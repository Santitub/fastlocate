...
def is_excluded(path):
    ...
    return any(os.path.abspath(excluded).lower() in path for excluded in EXCLUDED_PATHS)
...
def parallel_walk(start_dir='C:\\', use_threads=False, existing_index=None):
    to_scan = [start_dir]
...
def main():
    ...
    parser.add_argument('--path', default='C:\\', help='Ruta desde donde escanear (por defecto C:\\)')
    ...
    if args.update:
        if not is_admin():
            ...
            print("👉 Ejecuta el símbolo del sistema como administrador y vuelve a intentarlo.")
            sys.exit(1)
...
if __name__ == '__main__':
    freeze_support()  # Necesario para compatibilidad con multiprocesamiento en Windows
    main()
